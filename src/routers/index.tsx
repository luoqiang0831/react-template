...
// 包裹路由
function RouteWithRoutes(route: TRouteProps, key: string | number) {
  const T_Route = (
    <Route key={key} path={route.path as string} element={onAuth(route.component, { routes: route.routes })}>
      {
        // 如果有子路由
        Array.isArray(route.children) &&
          route.children.length > 0 &&
          route.children.map((childRoute: TRouteProps, index: number) => RouteWithRoutes(childRoute, index))
      }
    </Route>
  )
 ...
 * 404页面
 */
const NotFound = LoadableComp(() => import(/* webpackChunkName:'notfound' */ "@/pages/notfound"))
    ...
    >
      <Routes>
        <Route path="/" element={<Home />} />
        {
          ...
          /* @ts-ignore */
          routes?.map((route: TRouteProps, index) => RouteWithRoutes(route, index))
        }
        {/* <Route
                  path="/test"
